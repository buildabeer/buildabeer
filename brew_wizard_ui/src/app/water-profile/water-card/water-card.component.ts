...
export class WaterCardComponent implements OnInit {
  ...
  @Output()
  onWaterDelete = new EventEmitter();

  @Output()
  onProfileEdit = new EventEmitter();
  ...
  deleteCard(): void {
    var recipe_message: string = this.profile.recipe_count > 0 ? "This profile is used " + this.profile.recipe_count + " times, are you sure you want to delete it?" : "Are you sure you want to delete this profile?"
      ...
      this._waterProfileService.deleteWaterProfile(this.profile.id)
        .subscribe((res) => {
          if(this._router.url !== '/water_profiles') {
            ...
            this.onWaterDelete.emit({waterProfile: this.profile});
          }
        }, (error) => {
          if (error.status == "401") {
  ...
  editEvent(event): void {
    ...
    this.onProfileEdit.emit({profile: this.profile});
  }
}
