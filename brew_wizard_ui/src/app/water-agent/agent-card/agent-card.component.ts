...
export class AgentCardComponent implements OnInit {
  ...
  @Output()
  uponAgentDelete = new EventEmitter();
  ...
  deleteCard(): void {
    const recipe_message: string = this.agent.recipe_count > 0
    ? 'This agent is used ' + this.agent.recipe_count + ' times, are you sure you want to delete it?'
      ...
      this._agentService.deleteAgent(this.agent.id)
        .subscribe((res) => {
          if (this._router.url !== '/water_agents') {
            ...
            this.uponAgentDelete.emit({agent: this.agent});
          }
        }, (error) => {
          if (error.status === 401) {
  ...
  editEvent(event): void {
    ...
    this.uponAgentEdit.emit({agent: this.agent});
  }
}
