...
export class RecipeRunnerComponent implements OnInit {
  ...
  ph_collapse: boolean;
  ph_adjustment: number = 0;
  gravity_collapse: boolean;
  gravity_adjustment: number = 0;
  ...
  recalculateBoilTime(): void {
    var gallons = this._designer.inputConversion(this.boil_gallons, 'liquid') - this._designer.used_equipment.wl_boil;
    var new_time_hours = (gallons - this._designer.recipe.batch_size) / this._designer.used_equipment.boil_rate;
  ...
  addReminders(): void {
    var calendar_events: ICalendarEvent[] = [];
    var current_date: Date = new Date();
    var login_error: boolean = false;
    var other_errors: string[] = [];
    ...
    this.emailReminders.secondary_yeast.forEach((yeast, i) => {
      if (yeast) {
        var reminder_start_date = new Date();
    ...
    this.emailReminders.primary_misc.forEach((misc, i) => {
      if (misc) {
        var reminder_start_date = new Date();
        var add_days = this.primary_miscs[i].time;
    ...
    this.emailReminders.secondary_misc.forEach((misc, i) => {
      if (misc) {
        var reminder_start_date = new Date();
        var add_days = this.secondary_miscs[i].time;
    ...
    this.emailReminders.bottling_misc.forEach((misc, i) => {
      if (misc) {
        var reminder_start_date = new Date();
        var add_days = this.bottling_miscs[i].time;
  ...
  getMaltDisplay(usage: string): { name: string, quantity: number }[] {
    var malts: { name: string, quantity: number }[] = []
  ...
  setupBoilAlarms() {
    this.hopAlarms = {};
    var hops = this._designer.getHopByUsage('boil')
    hops.forEach((hop) => {
      var time = Math.floor(hop.time * 60)
    ...
    this.miscBoilAlarms = {};
    var miscs = this._designer.getUsedMiscellaneous('Boil');
    miscs.forEach((misc) => {
      var time = Math.floor(misc.time * 60)
  ...
  startInterval() {
    ...
    this.timerInterval = setInterval(() => {
      ...
      if (this.currentStep === Step.Boiling) {
        ...
        for(var key in this.miscBoilAlarms) {
          if(Number(key) >= this.timer) {
            this.playSound();
