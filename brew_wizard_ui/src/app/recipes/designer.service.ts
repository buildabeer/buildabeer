...
export class DesignerService {
  ...
  water_profile_select: boolean = true;
  water_profile: { style: string, color: string, hoppiness: string, strength: string } = { style: 'ale', color: 'pale', hoppiness: 'soft', strength: 'light' }
  recipe: IRecipe;
  ...
  used_water_ratio: number = 1;

  strike_amount: number = 0;
  so4_cl_ratio: number = 1;
  ra: number = 0;
  mash_ph: number = 5.5;
  ...
  mash_acid: boolean = false;
  sparge_acid: boolean = false;
  ...
  //checkbox vars
  match_water: boolean = true;
  match_agent: boolean = true;
  match_malt: boolean = true;
  match_hop: boolean = true;
  match_misc: boolean = true;
  match_yeast: boolean = true;
  ...
  loading_waters: boolean = true;
  loading_agents: boolean = true;
  loading_acids: boolean = true;
  loading_mashsteps: boolean = true;
  loading_malttypes: boolean = true;
  loading_malts: boolean = true;
  loading_hops: boolean = true;
  loading_miscs: boolean = true;
  loading_yeast: boolean = true;
  loading_styles: boolean = true;
  loading_equipment: boolean = true;
  loading_recipe: boolean = true;
  loading_measurements: boolean = true;
  loading_water_profiles: boolean = true;
          ...
          if(!this.used_waters[0]) {
            this.waters.some((wp) => {
              if(wp.name === 'Distilled Water') {
                ...
                this.recipe.recipe_waters_attributes.push({'water_id': wp.id, 'quantity': this.checkGallonsToLiters(this.estimateGallonsNeeded()), 'id': null, 'boil': false})
                return true;
              }
            })
          }
    ...
    if (new_recipe) {
      ...
      .subscribe(mashStepData => {
          ...
          mashStepData.forEach((step, i) => {
            this.recipe.recipe_mashes_attributes.push({name: step.name,
    ...
    this._styleService.getStyles()
      ...
      .subscribe(styleData => {
          ...
          if(!this.used_style) {
            ...
            this.recipe.carbonation_volumes = Math.round(((this.used_style.max_carb - this.used_style.min_carb) / 2 + this.used_style.min_carb) * 10) / 10
          }
          ...
          this._agentService.getAgents()
            ...
            .subscribe(agentData => {
                ...
                agentData.forEach((agent, i) => {
                  var new_agent = {'id': null, 'water_agent_id': agent.id, 'quantity': 0};
  ...
  onAddWaterProfile(add_water): void {
    ...
    if(!this.used_waters.includes(add_water) ) {
      ...
      if(this.addWaterGallons() < this.estimateGallonsNeeded()) {
        this.recipe.recipe_waters_attributes[this.recipe.recipe_waters_attributes.length - 1].quantity = this.checkGallonsToLiters(Math.round(10 * (this.estimateGallonsNeeded() - this.addWaterGallons())) / 10);
      }
    }
  }
  ...
  removeWater(remove_water): void {
    var index = this.used_waters.indexOf(remove_water);
    if (index > -1) {
  ...
  agentTitle(agent: string): string {
    var returnString: string = "No information available.";
    switch(agent) {
      case "main":
        returnString = "The warnings on this table are given as a general guideline for what is acceptable levels. There are times where it is acceptable not to match these levels."
        break;
      case "ph":
        returnString = "A log scale that measures the acidity or alkalinity of a solution. A 7 is neutral or balance, 1-6 are acidic, 8-12 are basic."
      ...
      case "ca":
        ...
        break;
      case "mg":
        returnString = "Secondary mineral of hardness. It is an enzyme cofactor and yeast nutrient. It accentuates beer flavor in the right portions."
        if(!this.agentPortion(agent)) {
          returnString += "\n\nThe usual levels are between 10 and 30. It should never be greater than 125, at that point it is a diuretic and cathartic"
        }
        break;
      case "na":
      ...
      case "so4":
        ...
        break;
      case "cl":
        returnString = "Enhances beer flavor and palate fullness. It increases the perception of sweetness, or mellowness. It also increases beer stability and improves clarity"
        if(!this.agentPortion(agent)) {
  ...
  getActiveAgents(): any {
    let activeAgents: { water_agent_id: number, id: number, name: string, quantity: number}[] = [];

    for(let i = 0; i < this.agents.length; i++) {
      if(this.recipe.recipe_water_agents_attributes[i].quantity > 0) {
        activeAgents.push({ "water_agent_id": this.recipe.recipe_water_agents_attributes[i].water_agent_id,
  ...
  estimatedPostBoilGallons(): number {
    ...
    if(this.used_equipment) {
      return this.inputConversion(this.recipe.batch_size, 'liquid') + this.used_equipment.wl_boil + (this.recipe.secondary_fermentation ? .1 : 0)
    }
  ...
  currentWater(): IWaterProfile {
    var total_gallons: number = this.addWaterGallons()
    var current_water: IWaterProfile = {
        id: 0,
        user_id: 0,
    ...
    this.used_waters.forEach((profile, i) => {
      var calcium_to_add = profile.calcium;
      var hco3final = profile.bicarbonate
      ...
      current_water.ph += (Math.pow(10, profile.ph) * this.inputConversion(this.recipe.recipe_waters_attributes[i].quantity, 'liquid') / total_gallons)
  ...
  waterDifference(): IWaterProfile {
    ...
    var current_water = this.currentWater();
  ...
  waterSumOfSquares(): number {
    var diff: IWaterProfile = this.waterDifference()
  ...
  matchWater(): void {
    var current_gallon_counts: any[];
    var best_index: number;
    var least_sos: number;
    var new_sos: number;
    ...
    while(this.addWaterGallons() < this.checkGallonsToLiters(this.estimateGallonsNeeded())) {
      ...
      this.recipe.recipe_waters_attributes[best_index].quantity = this.addPointOne(this.recipe.recipe_waters_attributes[best_index].quantity);
    }
  }
  ...
  matchAgents(): void {
    ...
    while(dirty) {
      ...
      this.recipe.recipe_water_agents_attributes.forEach((num, index) => {
        this.recipe.recipe_water_agents_attributes[index].quantity = this.addPointOne(this.recipe.recipe_water_agents_attributes[index].quantity);
        if (this.waterSumOfSquares() < least_sos) {
          least_sos = this.waterSumOfSquares();
          dirty = true;
        } else {
          this.recipe.recipe_water_agents_attributes[index].quantity = this.subtractPointOne(this.recipe.recipe_water_agents_attributes[index].quantity);
        }
  ...
  waterAutoOnChange(new_target = null): void {
    ...
    if (this.used_waters[1]) {
      this.used_water_ratio = 100 * this.recipe.recipe_waters_attributes[1].quantity / (this.recipe.recipe_waters_attributes[0].quantity + this.recipe.recipe_waters_attributes[1].quantity)
    }
    this.setStrikeAmount()
    if(!this.water_profile_select) {
      ...
      var water_needed = this.checkGallonsToLiters(this.estimateGallonsNeeded())
      var water_used = this.recipe.recipe_waters_attributes[0].quantity + (this.recipe.recipe_waters_attributes[1] ? this.recipe.recipe_waters_attributes[1].quantity : 0)
      if (water_used < water_needed) {
        this.recipe.recipe_waters_attributes[0].quantity = this.roundPointOne(this.recipe.recipe_waters_attributes[0].quantity + water_needed - water_used)
      }
    }
  }
  ...
  waterPercentChange(): void {
    var water_total = (this.recipe.recipe_waters_attributes[0].quantity + this.recipe.recipe_waters_attributes[1].quantity)
  ...
  onAddMalt(add_malt): void {
    ...
    this.recipe.recipe_malts_attributes.push({"id": null, "malt_id": add_malt.id,
  ...
  getSpargeInfo(): string {
    return "Sparging is the process of rinsing the grain to remove as much starch as possible.\n\n" +
      ...
      "Fly sparging is a process where the wort is slowly drained from the bottom while fresh water is added on top to rinse the grain as it travels downward."
  }

  getMashInfo(): string {
    return "It is optional to use different rests at different temperatures in order to active or deactivate specific enzymes at their highest efficiency.\n\n" +
      "Step mashing is where you heat the wort without applying direct heat on the wort/grain, but instead by adding water at specific temperatures and quanitites to raise the temperature.\n\n" +
      "Decoction mashing is where you remove quantities of wort/grain from the mash, bring it to a boil and add it back in to raise the temperature."
  }

  selectedMaltEvent(event): void {
    for(let i = 0; i < event.malts.length; i++) {
      this.used_malts.push(Object.assign({}, event.malts[i]));
      this.recipe.recipe_malts_attributes.push({id: null, malt_id: event.malts[i].id,
        color: event.malts[i].color, quantity: 0, malt_usage:
        ((this.maltTypes[event.malts[i].malt_type_id-1].name === "Grain" || this.maltTypes[event.malts[i].malt_type_id-1].name === "Adjunct") ?
              "Mash" : "Boil")
      })
    }
  }
  ...
  getMashWaterInfo(): { sparge_amount: number, step_info: { step_name: string, step_temp: number, step_time: number, water_amount: number, water_temp: number }[] } {
    var activate_steps: { step_name: string, step_temp: number, step_time: number, water_amount: number, water_temp: number }[] = []
    var strike_temp: number;
    var previous_temp: number;
    var total_water: number = 0;
    if (this.recipe.recipe_type === 'Extract') {
      let mash_step_temp = this.recipe.recipe_mashes_attributes[this.infusion_step].temperature;
      ...
      activate_steps.push({
        ...
        water_temp: Math.max((.2 / this.recipe.mash_ratio) * (this.inputConversion(mash_step_temp, 'temperature') - 72) + this.inputConversion(mash_step_temp, 'temperature'), this.inputConversion(mash_step_temp, 'temperature')),
        step_name: "Infusion",
        step_time: 20,
        step_temp: this.inputConversion(mash_step_temp, 'temperature'),
      });
      ...
      this.recipe.recipe_mashes_attributes.forEach((mash_step, i) => {
        if (mash_step.used) {
          var new_step: { step_name: string, step_temp: number, step_time: number, water_amount: number, water_temp: number } = { step_name: "", step_temp: 0, step_time: 0, water_amount: 0, water_temp: 0 };
          ...
          if (!strike_temp) {
            ...
            new_step.water_amount = (this.inputConversion(mash_step.temperature, 'temperature') - previous_temp) * (.2 * this.getTotalGrainWeight() + total_water) / (212 - this.inputConversion(mash_step.temperature, 'temperature'));
  ...
  getBeerColor(): string {
    var srm = this.inputConversion(this.getBeerColorValue(), 'color');
  ...
  getTotalGrainWeight(): number {
    ...
    this.used_malts.forEach((m, i) => {
      if(this.recipe.recipe_malts_attributes[i].malt_usage === 'Mash' && (this.maltTypes[m.malt_type_id-1].name === "Grain" || this.maltTypes[m.malt_type_id-1].name === "Adjunct")) {
  ...
  getEstimatedFG(): number {
    var highest_att: number = 0;
    this.used_yeast.forEach((yeast, i) => {
      var avg_att = yeast.min_attenuation + yeast.max_attenuation - yeast.min_attenuation;
  ...
  getPhDrop(chemical: string, currentPh: number, water_amount: number = null): number {
    ...
    var s_ra = .037 + .014 * mash_thickness;
    var change_amount = (chemical === 'lactic' ? 0.1 : 1);
    var quantity = change_amount;
  ...
  getGristAffect(): number {
    var grist_fraction: number[] = [];
    var acidity: number[] = [];
    var acidic_fraction = 0;
  ...
  getStrikeWaterAffect(): number {
    var percentage_distilled = 0;
    var mash_thickness = this.inputConversion(this.recipe.mash_ratio, 'ratio');
    mash_thickness = mash_thickness * 3.785 * 2.2 / 4; //(L/kg)
    var s_ra = .037 + .014 * mash_thickness;
    var nutrients = this.currentWater();
    ...
    var positive_ion_normality = normalities.calcium + normalities.magnesium + normalities.sodium
    var negative_ion_normality = normalities.sulfate + normalities.chloride + normalities.bicarbonate
    var ion_balance = positive_ion_normality - negative_ion_normality;
    var kt_ra = normalities.bicarbonate - normalities.calcium / 2.8 - normalities.magnesium / 5.6 + this.getAcidAffect();
    return s_ra * kt_ra;
  }

  getAcidAffect(): number {
    var acid_ounces: number = 0;
    var malt_alkalinity_meql: number = 0;
    ...
    var strength_percent = .03;
    var strike_volume_liters = this.inputConversion(this.recipe.mash_ratio, 'ratio') * 3.785 / 4 * this.getTotalGrainWeight();
    ...
    var acid_alkalinity_meql: number = 0;
  ...
  getChemicalAffect(acid_type: string, acid_amount: number): number {
    ...
    var malt_alkalinity_meql = -strength_percent * acid_ounces * 28.35 / 90.09 / strike_volume_liters * 1000
    ...
    var phosphoric_strength = .1;
    var lactic_strength = .88;
    var phosphoric_density = 1.05;
    var lactic_density = 1.21;

    var phosphoric_alkalinity_meql = -phosphoric_strength * phosphoric_density / 98 * 1000 * this.inputConversion(phosphoric_acid, 'liquid_agent') / strike_volume_liters;
    var lactic_alkalinity_meql = -lactic_strength * lactic_density / 90.09 * 1000 * this.inputConversion(lactic_acid, 'liquid_agent') / strike_volume_liters;
  ...
  onAddHop(add_hop): void {
    ...
    var newAddition = {hop_id: add_hop.id, id: null,
  ...
  individualIBU(hop, i, method: string): number {
    var add_ibu: number = 0;
    var utilization: number;
    var bigness: number;
    var boil_factor: number;
    ...
    if (method === 'tinseth') {
      ...
      add_ibu = (this.inputConversion(hop.quantity, 'hops') * utilization * (hop.alpha / 100) * 7489) / (this.estimatedPostBoilGallons() * (1 + Math.max(0, (this.getEstimatedOG() - 1.050) / .2)))
    } else if (method === 'garetz') {
      var desired_ibus = (this.calculateIBU("tinseth") + this.calculateIBU("rager")) / 2;
      var average_elevation: number = 2500;

      var cf: number = this.estimatedPostBoilGallons() / this.estimatePreGallons();
      var bg: number = (cf * (this.getEstimatedOG() - 1)) + 1;

      var gf: number = (bg - 1.050) / .2 + 1;
      var hf: number = (cf * desired_ibus / 260) + 1;
      var tf: number = (average_elevation / 550 * .02) + 1;

      var ca: number = gf * hf * tf;
  ...
  getHopByUsage(usage: string): { name: string, quantity: number, time: number, form: string }[] {
    var hops: { name: string, quantity: number, time: number, form: string }[] = []
  ...
  onAddMiscellaneous(add_misc): void {
    ...
    var newAddition = {miscellaneou_id: add_misc.id, id: null,
      quantity: (add_misc.amount / add_misc.batch_size) * this.recipe.batch_size, quantity_label: add_misc.amount_label, time: this._miscService.calculateTime(add_misc.time).time, time_label: this.capitalizeFirstLetter(this._miscService.calculateTime(add_misc.time).label),
  ...
  getUsedMiscellaneous(stage_used): { quantity: number, quantity_label: string, name: string, time: number, time_label: string }[] {
    var used_miscellaneous: { quantity: number, quantity_label: string, name: string, time: number, time_label: string }[] = []
  ...
  onAddYeast(add_yeast, stage): void {
    ...
    this.recipe.recipe_yeasts_attributes.push({id: null, yeast_id: add_yeast.id,
      quantity: 1, fermentation_stage: stage
    })
  }

  removeYeast(yeast): void {
    var index = this.used_yeast.indexOf(yeast);
  ...
  selectedYeastEvent(event, stage): void {
    for(let i = 0; i < event.yeast.length; i++) {
      let add_yeast = Object.assign({}, event.yeast[i])
  ...
  optimalYeastTemperatureCheck(stage): boolean {
    var temperature = this.inputConversion((stage === 1) ? this.inputConversion(this.recipe.primary_ferm_temp, 'temperature') : this.inputConversion(this.recipe.secondary_ferm_temp, 'temperature'), 'temperature');
    var range = this.optimalYeastTemperature(stage);
  ...
  optimalYeastTemperatureMessage(stage): string {
    ...
    if (range.min > range.max) {
      ...
      return "It is suggested to ferment at a temperature within the yeasts optimal temperature range of " + range.min + " - " + range.max + " degrees F. Temperatures outside of the optimal range can lead to failed fermentation and off flavors."
    }
  }

  getUsedYeast(stage_used): { quantity: number, name: string, lab: string }[] {
    var used_yeast: { quantity: number, name: string, lab: string }[] = []
    this.used_yeast.forEach((yeast, i) => {
      if (this.recipe.recipe_yeasts_attributes[i].fermentation_stage === stage_used) {
        used_yeast.push({ "quantity": this.recipe.recipe_yeasts_attributes[i].quantity,
  ...
  getVolumesQuantity(): number {
    ...
    var sugar_factor: number = 4;
    var sugar_volume_factor: number = 1;
    ...
    switch(this.recipe.carbonation_with) {
      ...
      case "tablesugar":
        ...
        break;
      case "gyle":
        var carbonation_beer = (1.013) * Math.pow(2.71828182845904, (-10.73797 + (2617.25 / (this.fahrToCels(this.inputConversion(this.recipe.storage_temperature, 'temperature')) + 273.15)))) * 10;
        quantity = 1.95 * 4 * this.inputConversion(this.recipe.batch_size, 'liquid') * (this.recipe.carbonation_volumes - carbonation_beer / 2) / (this.getEstimatedOG() - this.getEstimatedFG()) / 1000
        return this.checkGallonsToLiters(quantity, true);
    }

    var dissolved_co2 = ((-0.9753) * Math.log(this.inputConversion(this.recipe.storage_temperature, 'temperature')) + 4.9648);
    var co2_factor = this.recipe.carbonation_volumes - dissolved_co2;
    var vol_factor = this.inputConversion(this.recipe.batch_size, 'liquid') * 3.78541;
    quantity = co2_factor * sugar_factor * vol_factor * sugar_volume_factor / (this.measurement.malts === 'us' ? 28.34952 : 1); //28.34952 is gram to oz
  ...
  recipeScale(new_batch_size): void {
    var old_water_amount = this.estimateGallonsNeeded()
    ...
    var new_water_amount = this.estimateGallonsNeeded()
  ...
  getIngredientList(): string {
    ...
    this.used_malts.forEach((malt, i) => {
      ingredients.push(
        ...
        this.roundPointOne( this.recipe.recipe_malts_attributes[i].quantity) + ' ' + ( this.measurement.malts === 'us' ? 'lbs' : 'kg' ) + '\t' +
    ...
    this.used_miscellaneous.forEach((misc, i) => {
      ingredients.push(
        ...
        this.roundHundredth(this.recipe.recipe_miscellaneous_attributes[i].quantity) + ' ' + ( this.recipe.recipe_miscellaneous_attributes[i].quantity_label ) + '\t' +
    ...
    this.used_hops.forEach((hop, i) => {
      ingredients.push(
        ...
        ( hop.name ) + " " + (( this.displayHopInfo(this.recipe.recipe_hops_attributes[i].usage, this.recipe.recipe_hops_attributes[i].time) ))
      )
    })
    this.used_yeast.forEach((yeast, i) => {
      ingredients.push(
    ...
    this.used_waters.forEach((water, i) => {
      ingredients.push(
        'Water\t' +
        this.roundPointOne(this.recipe.recipe_waters_attributes[i].quantity) + ' ' + ( (this.measurement.liquid === 'us' ? "gallons" : "liters") ) + '\t' +
        ( water.name )
      )
    })
  ...
  formRecipeJson(): {} {
    var recipe_json = { recipe: {} };
    var recipe = this.getRecipeCopy();
    ...
    recipe.recipe_hops_attributes.forEach((hop, i) => {
      ...
      recipe_json.recipe['hops'].push({ name: this.used_hops[i].name, version: 1,
        alpha: hop.alpha, amount: this.convertToXml(hop.quantity, 'hops'), use: usage, time: hop.time, form: hop.form === 'Pellet' ? 'Pellet' : 'Plug' })
    })
    ...
    var mash_info = this.getMashWaterInfo();
    var mash_type = ""
    if(recipe.mash_type === 'none') {
      mash_type = "Infusion";
  ...
  parseXml(my_json: {}, key_name: string = null): string[] {
    ...
    for (var key in my_json) {
      if (my_json[key].constructor === Array) {
        ...
        parsed_xml.push("<" + (key_name === null ? key : key_name).toUpperCase() + ">" + my_json[key] + "</" + (key_name === null ? key : key_name).toUpperCase() + ">")
      }
    }
    return parsed_xml;
  }
  ...
  exportXml(): void {
    var recipe_json: {} = this.formRecipeJson();
    var xml_file = ["<?xml version=\"1.0\" encoding=\"ISO-8859-1\"?>", "<RECIPES>"];

    xml_file = [...xml_file, ...this.parseXml(recipe_json)];
    xml_file.push("</RECIPES>");

    let blob: any = new Blob([xml_file.join('')], { type: 'xml' });
  ...
  loadNode(node): string[] {
    ...
    switch(node.nodeName) {
      ...
      case "BATCH_SIZE":
        ...
        break;
      case "BOIL_TIME":
        this.recipe.boil_time = parseInt(node.innerHTML);
        break;
      case "NOTES":
      ...
      case "CARBONATION":
        ...
        break;
      case "PRIMARY_AGE":
        this.recipe.primary_ferm_days = parseInt(node.innerHTML);
        break;
      case "PRIMARY_TEMP":
        this.recipe.primary_ferm_temp = this.convertFromXml(parseFloat(node.innerHTML), 'temperature');
        break;
      case "SECONDARY_AGE":
        this.recipe.secondary_ferm_days = parseInt(node.innerHTML);
        break;
      case "SECONDARY_TEMP":
      ...
      case "CARBONATION_TEMP":
        ...
        break;
      case "EQUIPMENT":
        let name = node.getElementsByTagName('NAME')[0].innerHTML;
      ...
      case "STYLE":
        this.styles.some((style) => {
          if (style.category_number === parseInt(node.getElementsByTagName('CATEGORY_NUMBER')[0].innerHTML) &&
        ...
        if (!found) {
          error_list.push("The style " + node.getElementsByTagName('CATEGORY_NUMBER')[0].innerHTML + node.getElementsByTagName('STYLE_LETTER')[0].innerHTML + " was not found.");
        }
        break;
      case "HOPS":
        let hop_list = node.getElementsByTagName("HOP");
        for (var i = 0; i < hop_list.length; i++) {
          let hop_node = hop_list[i]
          found = false;
          this.hops.some((hop) => {
            if(hop.name === hop_node.getElementsByTagName('NAME')[0].innerHTML) {
              this.used_hops.push(hop);
              var new_hop = { hop_id: hop.id, id: null,
                  ...
                  time: parseInt(hop_node.getElementsByTagName('TIME')[0].innerHTML),
        ...
        break;
      case "FERMENTABLES":
        let malt_list = node.getElementsByTagName("FERMENTABLE");
        for (var i = 0; i < malt_list.length; i++) {
          let malt_node = malt_list[i]
          found = false;
          this.malts.some((malt) => {
            if(malt.name === malt_node.getElementsByTagName('NAME')[0].innerHTML) {
              this.used_malts.push(malt);
              var new_malt = { malt_id: malt.id, id: null,
        ...
        break;
      case "YEASTS":
        let yeast_list = node.getElementsByTagName("YEAST");
        for (var i = 0; i < yeast_list.length; i++) {
          let yeast_node = yeast_list[i]
          found = false;
          this.yeast.some((yeast) => {
            if((yeast_node.getElementsByTagName('PRODUCT_ID')[0] && yeast.product_id === yeast_node.getElementsByTagName('PRODUCT_ID')[0].innerHTML) ||
                    (!yeast_node.getElementsByTagName('PRODUCT_ID')[0] && yeast.name === yeast_node.getElementsByTagName('NAME')[0].innerHTML)) {
              this.used_yeast.push(yeast);
              var new_yeast = { yeast_id: yeast.id, id: null,
                  quantity: this.convertFromXml(parseFloat(yeast_node.getElementsByTagName('AMOUNT')[0].innerHTML), 'yeasts'),
                  fermentation_stage: yeast_node.getElementsByTagName('ADD_TO_SECONDARY')[0] ? (yeast_node.getElementsByTagName('ADD_TO_SECONDARY')[0].innerHTML === "TRUE" ? 2 : 1) : 1 }
        ...
        break;
      case "WATERS":
        let water_list = node.getElementsByTagName("WATER");
        for (var i = 0; i < water_list.length; i++) {
          let water_node = water_list[i]
          found = false;
          this.waters.some((water) => {
            if(water.name === water_node.getElementsByTagName('NAME')[0].innerHTML) {
              this.used_waters.push(water);
              var new_water = { water_id: water.id, id: null,
                  quantity: this.convertFromXml(parseFloat(water_node.getElementsByTagName('AMOUNT')[0].innerHTML), 'liquid'), boil: false }
        ...
        break;
      case "MASH_STEPS":
        let mash_step_list = node.getElementsByTagName("MASH_STEP");
        ...
        for (var i = 0; i < mash_step_list.length; i++) {
          let mash_step_node = mash_step_list[i]
          found = false;
          this.recipe.recipe_mashes_attributes.some((mash_step, i) => {
            if(mash_step.name === mash_step_node.getElementsByTagName('NAME')[0].innerHTML || (mash_step_node.getElementsByTagName('NAME')[0].innerHTML === "Infusion" && i === this.infusion_step)) {
              mash_step.used = true;
        ...
        break;
      case "MISCS":
        let misc_step_list = node.getElementsByTagName("MISC");
        for (var i = 0; i < misc_step_list.length; i++) {
          let misc_step_node = misc_step_list[i]
          found = false;

          if(misc_step_node.getElementsByTagName('TYPE')[0].innerHTML === 'Water Agent') {
            let agent_name = misc_step_node.getElementsByTagName('NAME')[0].innerHTML;
              ...
              if(agent.name === agent_name) {
                this.recipe.recipe_water_agents_attributes.some((used_agent) => {
                  if(used_agent.water_agent_id === agent.id) {
                    used_agent.quantity = this.convertFromXml(parseFloat(misc_step_node.getElementsByTagName('AMOUNT')[0].innerHTML), 'agents');
                    found = true;
                    return true;
                  }
                })
            ...
            let misc_name = misc_step_node.getElementsByTagName('NAME')[0].innerHTML;
  ...
  parseXmlImport(recipe) {
    ...
    //run through import
    let child_nodes = recipe.childNodes;
